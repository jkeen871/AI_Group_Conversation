...
class EditHelperPersonalities(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Edit Helper Personalities")
    ...
    def initUI(self):
        ...
        # Left side: List of helper personalities
        ...
        new_personality_button = QPushButton("Add New Helper Personality")
        ...
        remove_personality_button = QPushButton("Remove Helper Personality")
    ...
    def loadPersonalities(self):
        ...
        for name in HELPER_PERSONALITIES.keys():
            self.personality_list.addItem(name)

    def loadPersonalityDetails(self, item):
        name = item.text()
        personality = HELPER_PERSONALITIES[name]
    ...
    def saveChanges(self):
        ...
        if not current_item:
            QMessageBox.warning(self, "No Selection", "Please select a helper personality to save changes.")
            return
        ...
        if current_name != new_name and new_name in HELPER_PERSONALITIES:
            QMessageBox.warning(self, "Name Exists", "A helper personality with this name already exists.")
            return

        HELPER_PERSONALITIES[new_name] = {
            'name': new_name,
        ...
        if current_name != new_name:
            del HELPER_PERSONALITIES[current_name]
    ...
    def addNewPersonality(self):
        new_name = "New Helper Personality"
        counter = 1
        while new_name in HELPER_PERSONALITIES:
            new_name = f"New Helper Personality {counter}"
            counter += 1
    ...
    def removePersonality(self):
        current_item = self.personality_list.currentItem()
        if not current_item:
            QMessageBox.warning(self, "No Selection", "Please select a helper personality to remove.")
            return

        name = current_item.text()
        reply = QMessageBox.question(self, 'Remove Helper Personality',
                                     f"Are you sure you want to remove '{name}'?",
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.No)

        if reply == QMessageBox.Yes:
            del HELPER_PERSONALITIES[name]
            self.saveToFile()
            self.loadPersonalities()
            self.clearPersonalityDetails()
            QMessageBox.information(self, "Removed", f"Helper personality '{name}' has been removed.")
    ...
    def saveToFile(self):
        ...
        # Find the start and end of the HELPER_PERSONALITIES dictionary
        start_index = next(i for i, line in enumerate(lines) if line.strip().startswith("HELPER_PERSONALITIES = {"))
        end_index = next(i for i in range(start_index + 1, len(lines)) if lines[i].strip() == "}")

        # Convert the HELPER_PERSONALITIES dictionary to a formatted string
        personalities_str = "HELPER_PERSONALITIES = {\n"
        for name, personality in HELPER_PERSONALITIES.items():
        ...
        # Replace the old HELPER_PERSONALITIES dictionary with the new one
...
if __name__ == '__main__':
    ...
    dialog = EditHelperPersonalities()
    dialog.show()
    sys.exit(app.exec_())
